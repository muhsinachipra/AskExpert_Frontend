...
const ContactList = ({ conversation, currentUserId }: { conversation: IConversation, currentUserId: string }) => {
    ...
    return (
        <div key={conversation._id} className="flex items-center mb-4 cursor-pointer hover:bg-gray-100 p-2 rounded-md" onClick={() => handleChat(expertData?._id || '')} >
            <div className="relative w-12 h-12 bg-gray-300 rounded-full mr-3">
                <img src={expertData?.profilePic} alt="User Avatar" className="w-12 h-12 rounded-full" />
                {/* {contact.isOnline && (
